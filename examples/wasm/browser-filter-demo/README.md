...
- servo id: `0`
- multi-turn: checked
- zero hold seconds: `0.65`

`zero hold seconds` controls how long a raw zero must remain stable before the
filter accepts it as a real zero. Increase it if power-cycle startup still leaks
a short zero glitch; decrease it if you need intentional zero positions to be
accepted faster.
